...
private:
  ...
  const QDir m_DiamoneumDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfDiamoneumBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _Diamoneum) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo DiamoneumBlockchainInfo() const;
